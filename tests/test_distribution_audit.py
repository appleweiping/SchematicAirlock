...
import schematic_airlock.distribution_audit as distribution_audit
from schematic_airlock.distribution_audit import (
...
def test_auditors_accept_canonical_regular_files() -> None:
    with tar_archive(
        [
            ("schematic_airlock-1/", b"", tarfile.DIRTYPE),
            ("schematic_airlock-1/src/", b"", tarfile.DIRTYPE),
            ("schematic_airlock-1/src/module.py", b"value = 1\n", None),
        ]
    ) as archive:
        assert audit_sdist(archive, expected_root="schematic_airlock-1") == {"src/module.py"}
    with zip_archive(
        [
            ("schematic_airlock/", b"", stat.S_IFDIR | 0o755),
            ("schematic_airlock/module.py", b"value = 1\n", stat.S_IFREG | 0o644),
        ]
    ) as archive:
        assert audit_wheel(archive) == {"schematic_airlock/module.py"}
...
def test_sdist_can_require_the_exact_release_root() -> None:
    with (
        ...
        audit_sdist(archive, expected_root="schematic_airlock-0.4.1")
